            ...
            <div id="ex0" class="container-fluid">
              ...
              <div id="ex4" class="container-fluid">
                <div class="container bgcl2">
                  ...
                  ?>
                  <div id="plable" class="text-center">PRIVATE CHAT</div>
                  <div class="row2 bgcls2">
                      <div class="row bgcl bgclx">

                          <div id="" class="col-sm-4 my-auto text-center">
                            <span class="my-auto txtsize">
                              <a href="profile.php?id=<?php echo $user['id']?>" class="click2x">to login: <?php echo $user['login']?></a>
                            </span>
                          </div>
                          ...
                          <div id="" class="col-sm-4 list my-auto text-center">
                            <span class="my-auto">to id: <?php echo $user['id']?></span>
                          </div>
...
function send()
{
  ...
  // Отсылаем паметры
     $.ajax({
              type: "POST",
              url: "worldchatadd.php?id=<?php $result = mysqli_query($sql, "SELECT * FROM `users` WHERE id = '".$_GET['id']."' limit 1 ");
                                              $user = mysqli_fetch_assoc($result); 
                                              echo $user['id'];
...
function load_messes()
  {
    $.ajax({
                type: "POST",
                url:  "worldchatload.php?id=<?php $result = mysqli_query($sql, "SELECT * FROM `users` WHERE id = '".$_GET['id']."' limit 1 ");
                                              $user = mysqli_fetch_assoc($result); 
                                              echo $user['id'];
